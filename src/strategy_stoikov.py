...
class StoikovStrategy:
    ...
    '''
    def __init__(self, delay: float, gamma: float, sigma: float, kappa: float, hold_time:Optional[float] = None) -> None:
        '''
            Args:
    ...
    def run(self, sim: Sim ) ->\
        ...
        while True:
            ...
            if receive_ts - prev_time >= self.delay:
                ...
                reservation_price = mid_price - self.gamma * self.sigma * self.sigma * sim.get_inventory() * delta_t
                #calculate agent's spread
                spread = self.gamma * self.sigma * self.sigma * delta_t + (2/self.gamma) * np.log(1 + (self.gamma/self.kappa))
