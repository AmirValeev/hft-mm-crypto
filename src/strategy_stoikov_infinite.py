...
class StoikovStrategy:
    ...
    '''
    def __init__(self, delay: float, gamma: float, sigma: float, kappa: float, A: float, hold_time:Optional[float] = None) -> None:
        '''
            Args:
                ...
                sigma(float): asset volatility, might be dynamic
                kappa(float): Order book liquidity parameter, for time nornmalizing
        '''
        ...
        self.T_begin = 1655942402250125991
        self.T_end = 1655943545128514890
    ...
    def run(self, sim: Sim ) ->\
        ...
        while True:
            ...
            if receive_ts - prev_time >= self.delay:
                ...
                delta_t = 1 - (receive_ts - self.T_begin)/(self.T_end - self.T_begin)
                
                ask_price = mid_price + 1/self.gamma * np.log(1 + self.gamma / self.kappa) - (sim.get_inventory() - 1/2) * (self.sigma**2 * self.gamma / (2 * self.kappa * self.A) * (1 + self.gamma/self.kappa)**(1 + self.kappa/self.gamma))**.5
                bid_price = mid_price - (1/self.gamma * np.log(1 + self.gamma / self.kappa) + (sim.get_inventory() + 1/2) * (self.sigma**2 * self.gamma / (2 * self.kappa * self.A) * (1 + self.gamma/self.kappa)**(1 + self.kappa/self.gamma))**.5)
